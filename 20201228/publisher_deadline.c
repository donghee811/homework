...
static void on_offered_deadline_missed(const dds_DataWriter* a_writer, const dds_OfferedDeadlineMissedStatus* status){
...
int main(int argc, char** argv) {
	...
	writer_qos.deadline.period.nanosec = 500 * 1000 * 1000;
	// 리스너생성
	dds_DataWriterListener dwl = {0,};
	dwl.on_offered_deadline_missed = on_offered_deadline_missed;

	// Create a DataWriter entity.
	dds_DataWriter* dw = dds_Publisher_create_datawriter(pub, topic, &writer_qos, &dwl, dds_OFFERED_DEADLINE_MISSED_STATUS);
	if(dw == NULL)
		return 13;
