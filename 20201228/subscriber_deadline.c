...
static void on_requested_deadline_missed(const dds_DataReader* a_reader, const dds_RequestedDeadlineMissedStatus* status){
...
int main(int argc, char** argv) {
	...
	reader_qos.deadline.period.nanosec = 500 * 1000 * 1000;

	// 리스너 생성
	dds_DataReaderListener drl = {0,};
	drl.on_requested_deadline_missed = on_requested_deadline_missed;

	// Create a DataReader entity.
	dds_DataReader* dr = dds_Subscriber_create_datareader(sub, topic, &reader_qos, &drl, dds_REQUESTED_DEADLINE_MISSED_STATUS);
	if(dr == NULL)
		return 13;
